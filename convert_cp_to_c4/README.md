...
1. Выполните подключение к Check Point R77.30 или R80.20 по SFTP и скачайте файлы конфигурации в кодировке UTF-8 (файлы с правилами в формате `*.fws` и объектами в формате `*.с`).
...
9. Выберите файл конфигурации в универсальном формате `*.json`, сгенерированный ранее скриптом `convert_cp_to_c4.py`.
...
17. Выберите файл конфигурации в универсальном формате `*.json`, сгенерированный ранее скриптом `convert_cp_to_c4.py`.
...
17. Для сетевых объектов типа `network` при импорте правил из Check Point в К4 не осуществляется перенос следующих параметров: 
    ...
    - Network address (IPv6);
    - Prefix (IPv6).
18. Для сетевых объектов типа `machines_range` при импорте правил из Check Point в К4 не осуществляется перенос следующих параметров: 
    - NAT;
    - Color;
    - First IPv6 address;
    - Last IPv6 address.
