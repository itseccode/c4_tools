...
| `c4_excel_import`       | Инструмент для импорта правил фильтрации (FW) и правил трансляции (NAT) через промежуточный файл `*.xlsm'   | Да  | Только совместно с `c4_lib`                                                |
...
В случае возникновения проблем при использовании инструментов для получения необходимой диагностической информации рекомендуется использовать метод `print_debug` библиотеки `c4_lib`.
